...
select bidder_id,100 *(sum(bid_status='Won') )/count(bid_status in ('Bid','Lost','Won','Cancelled')) as Percentage_of_winning
from ipl_bidding_details
group by bidder_id
order by Percentage_of_winning desc;

#2.	Display the number of matches conducted at each stadium with stadium name,city from the database.

select stadium_name,city,count(ims.match_id) as Number_of_matches_each_stadium
from ipl_stadium ista
inner join ipl_match_schedule ims
on ista.stadium_id=ims.stadium_id
where ims.status='Completed'
group by stadium_name
order by count(ims.match_id) desc

#3.	In a given stadium, what is the percentage of wins by a team which has won the toss?


select ista.stadium_id,ista.stadium_name,100*(sum(im.toss_winner=im.match_winner)/count(im.match_id) )as Percentage_Toss_Winners
from ipl_match im
inner join  ipl_match_schedule ims
on im.match_id=ims.match_id
inner join ipl_stadium ista 
on ims.stadium_id=ista.stadium_id
group by ista.stadium_name
order by Percentage_Toss_Winners desc
#4.	Show the total bids along with bid team and team name.


select count(bid_status) as Total_bids,bid_team,it.team_name
...
select it.team_name,sum(matches_played) as Total_Played,
sum(matches_won) as Total_Won ,sum(matches_lost) as Total_lost
...
select itp.player_id,ip.player_name,player_role
...
select team_name,count(itp.player_role) as Count_All_rounder
from ipl_team it
inner join ipl_team_players itp
on it.team_id=itp.team_id
where itp.player_role='All-Rounder' 
group by team_name
order by Count_All_rounder desc
limit 5;
